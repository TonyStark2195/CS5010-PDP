...
public abstract class AbstractMaze implements Maze {
  ...
  protected List<Room> caveTunnels = new ArrayList<>();
  ...
  public void generateMazeObjects(long seed, int percentBats, int percentPits)
    ...
    int[] bats =
        new Random(seed + 2).ints(0, listCaves.size()).distinct().limit(numSuperBats).toArray();

    int[] pits = new Random(seed + 1).ints(0, listCaves.size()).distinct().limit(numPits).toArray();
  ...
  protected Room dfsArrow(Room currentRoom, String action, int limit) {
    ...
    while (!stackRoom.isEmpty() && count < limit) {
      ...
      if (!visitedRooms.contains(temp)) {
        visitedRooms.add(temp);
        if (temp.getRoomType() == RoomType.CAVE) {
          if (temp.getNeighbors().get(action) != null) {
      ...
      for (Character ch : characters) {
        if (ch.getName() == CharacterType.SUPER_BAT) {
          ...
          if (random.nextDouble() < 0.5) {
            ...
            int[] newIndex =
                new Random().ints(0, totalListCaves.size()).distinct().limit(1).toArray();
...
//          this.caveTunnels.add(destination);
          for (Room[] roomX : this.mazeRooms) {
            for (Room roomY : roomX) {
              this.caveTunnels.add(roomY);
            }
          }
  ...
  @Override
  public List<Room> getCaveTunnels() {
    return this.caveTunnels;
  }
