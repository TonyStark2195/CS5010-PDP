    ...
    for (int x = 0; x < this.numColumns; x++) {
      for (int y = 0; y < this.numRows; y++) {
        if (x - 1 >= 0) {
          ...
          Set<Room> tempSet = new HashSet<>();
          tempSet.add(this.mazeRooms[x][y]);
          tempSet.add(this.mazeRooms[this.numColumns - 1][y]);
          if (!this.wallList.contains(tempSet)) {
            this.wallList.add(tempSet);
          }
        }
        if (y - 1 >= 0) {
          ...
          Set<Room> tempSet = new HashSet<>();
          tempSet.add(this.mazeRooms[x][y]);
          tempSet.add(this.mazeRooms[x][this.numRows - 1]);
          if (!this.wallList.contains(tempSet)) {
            this.wallList.add(tempSet);
          }
        }
    ...
    for (int x = 0; x < this.numColumns; x++) {
      for (int y = 0; y < this.numRows; y++) {
        ...
        if (x - 1 >= 0) {
          ...
          Set<Room> tempWall = new HashSet<>();
          tempWall.add(this.mazeRooms[x][y]);
          tempWall.add(this.mazeRooms[this.numColumns - 1][y]);
          if (!this.savedList.contains(tempWall)) {
            tempSet.add(this.mazeRooms[this.numColumns - 1][y]);
            neighbors.put("West", this.mazeRooms[this.numColumns - 1][y]);
          }
        }
        if (y - 1 >= 0) {
          Set<Room> tempWall = new HashSet<>();
          ...
          Set<Room> tempWall = new HashSet<>();
          tempWall.add(this.mazeRooms[x][y]);
          tempWall.add(this.mazeRooms[x][this.numRows - 1]);
          if (!this.savedList.contains(tempWall)) {
            tempSet.add(this.mazeRooms[x][this.numRows - 1]);
            neighbors.put("North", this.mazeRooms[x][this.numRows - 1]);
          }
        }
        if (x + 1 <= this.numColumns - 1) {
          Set<Room> tempWall = new HashSet<>();
